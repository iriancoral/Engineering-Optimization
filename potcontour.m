...
g2_grid   = zeros(Nh, Nr);   % drainage
g3_grid   = zeros(Nh, Nr);   % stability
...
for i = 1:Nh
    for j = 1:Nr
        ...
        [Vmat, Vpot, ~, sigma_max, A_base, ~, ~] = potanalysis( ...
            r1_ij, h_ij, theta_wall, t, rho_mat, rho_soil, K0, g_acc, sigma_allow, cost_mat);

        Vmat_grid(i,j) = Vmat;
        ceq_grid(i,j)  = Vpot - V_required;
        g1_grid(i,j)   = sigma_max - sigma_allow;
        g2_grid(i,j)   = A_drain_min - A_base;
        g3_grid(i,j)   = h_ij - stab_ratio * r1_ij;
    end
end
...
% -------------------------------------------------------
infeas = (g1_grid > 0) | (g2_grid > 0) | (g3_grid > 0);
...
contour(r1_mm, h_mm, g2_grid, [0 0], 'm--', 'LineWidth', 2.0);   % drainage
contour(r1_mm, h_mm, g3_grid, [0 0], 'b--', 'LineWidth', 2.0);   % stability
...
h_leg(3) = plot(NaN, NaN, 'm--', 'LineWidth', 2.0, 'DisplayName', 'g_2: drainage limit');
h_leg(4) = plot(NaN, NaN, 'b--', 'LineWidth', 2.0, 'DisplayName', 'g_3: stability limit');
...
% For each r1, compute h analytically from volume equation,
% then evaluate objective and check constraints.
% -------------------------------------------------------
...
for j = 1:N_line
    ...
    % With fixed angle: r2 = r1 + h*tan(th), so Vpot is cubic in h.
    % Solve: (pi*h/3)*(r1^2 + r1*(r1+h*tan) + (r1+h*tan)^2) = V_req
    % Rearranged: (pi*tan^2/3)*h^3 + (pi*tan*r1)*h^2 + (pi*r1^2)*h - V_req = 0
    a   = tan(th);
    p3  = pi * a^2 / 3;
    p2  = pi * a * r1j;
    p1  = pi * r1j^2;
    ...
    [Vmat, ~, ~, sigma_max, A_base, ~, ~] = potanalysis( ...
        r1j, h_sol, theta_wall, t, rho_mat, rho_soil, K0, g_acc, sigma_allow, cost_mat);

    Vmat_line(j) = Vmat * 1e6;   % cm3

    g1_line(j) = sigma_max - sigma_allow;
    g2_line(j) = A_drain_min - A_base;
    g3_line(j) = h_sol - stab_ratio * r1j;

    % Feasible if all inequality constraints satisfied
    feas_line(j) = (g1_line(j) <= 0) && (g2_line(j) <= 0) && ...
                   (g3_line(j) <= 0) && (r1j >= r1_min) && (r1j <= r1_max);
end
...
if ~isnan(Vmat_min_est)
    r1_opt_est = r1_line(idx_min) * 1000;
    h_opt_est  = h_line_sol(idx_min) * 1000;
    ...
    fprintf('  Vmat = %.4f cm3\n',  Vmat_min_est);
    fprintf('  Active bounds: check constraint values below\n');
    fprintf('  g1 (stress)   = %.4e  (<=0 ok)\n', g1_line(idx_min));
    fprintf('  g2 (drainage) = %.4e  (<=0 ok)\n', g2_line(idx_min));
    fprintf('  g3 (stability)= %.4e  (<=0 ok)\n', g3_line(idx_min));
end
...
for j = 2:N_line
    if ~isnan(g2_line(j)) && ~isnan(g2_line(j-1))
        if sign(g2_line(j)) ~= sign(g2_line(j-1))
            xline(r1_line(j)*1000, 'm:', 'LineWidth', 1.5);
        end
    end
    if ~isnan(g3_line(j)) && ~isnan(g3_line(j-1))
        if sign(g3_line(j)) ~= sign(g3_line(j-1))
